...
export default function CartCard({ product: { title, image: { src, alt }, options: { size, material }, cartQuantity, price }, id, removeButton = true, bg = true, disabled = false }) {
   ...
   const [cartQty, setCartQty] = useState(cartQuantity)
   ...
   return (
      <div className={`${bg ? 'bg-theme-grey-900' : ''} flex items-center py-2 md:px-2  text-white w-full`}>
         ...
         <div className="flex flex-col flex-1">
            <div className="flex text-sm justify-between leading-tight">
               ...
               <div className="flex items-baseline">
                  {!disabled && <input value={cartQty} onChange={(e) => editQuantity(e.target.value)} onBlur={handleMinQuantity} className="bg-theme-grey-800 rounded w-6 h-4 text-center leading-4" type="number" min="1" step="1" disabled={disabled} />}
                  {disabled && <input value={cartQuantity} className="bg-theme-grey-800 rounded w-6 h-4 text-center leading-4" type="number" disabled={true} />}
